...
```bash
git clone https://github.com/ArmandFS/rbnz-fintech-rag.git
cd rbnz-fintech-rag/rag
```
...
For Gemini embeddings and Gemini LLM answering:
...
For retrieval-only local development, use:
...
Create a local documents folder:

```bash
mkdir -p documents
```

Place RBNZ PDF files inside:
